...
const unsigned long toRange = 1<<11;
...
static void BM_RowBase(benchmark::State& state) {
...
BENCHMARK(BM_RowBase)->RangeMultiplier(2)->Range(fromRange, toRange);


static void BM_ColumnBase(benchmark::State& state) {
...
BENCHMARK(BM_ColumnBase)->RangeMultiplier(2)->Range(fromRange, toRange);
...
static void BM_RandomBase(benchmark::State& state) {
    const size_t n = state.range(0);
    Matrix m(n);
    for (auto _ : state) {
        for (size_t i = 0; i < m.size(); i++) {
...
BENCHMARK(BM_RandomBase)->RangeMultiplier(2)->Range(fromRange, toRange);
